...
class SpatialGradient(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.mode: str = mode
        kernel: torch.Tensor = get_spatial_gradient_kernel2d(mode, order, coord)
        if self.normalized:
            kernel = normalize_kernel2d(kernel)
        # Pad with "replicate for spatial dims, but with zeros for channel
        self.spatial_pad = [kernel.size(1) // 2,
                            kernel.size(1) // 2,
                            kernel.size(2) // 2,
                            kernel.size(2) // 2]
        # Prepare kernel
        kernel: torch.Tensor = kernel.to(device).to(dtype)
        kernel: torch.Tensor = kernel.unsqueeze(1).unsqueeze(1)
        kernel: torch.Tensor = kernel.flip(-3)
    ...
    def forward(self, inp: torch.Tensor) -> torch.Tensor:  # type: ignore
        ...
        padded_inp: torch.Tensor = torch.nn.functional.pad(inp.float().reshape(b * c, 1, h, w),
