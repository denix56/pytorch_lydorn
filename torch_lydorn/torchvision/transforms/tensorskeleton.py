...
class Skeleton:
    def __init__(self, coordinates=None, paths=None, degrees=None):
        if coordinates is None:
            self.coordinates = np.empty((0, 2), dtype=float)
        else:
        ...
        if paths is None:
            ...
            self.paths = paths
        if degrees is None:
            self.degrees = np.empty(0, dtype=int)
        else:
...
class Paths:
    def __init__(self, indices=None, indptr=None):
        ...
        if indptr is None:
            self.indptr = np.empty(0, dtype=int)
        else:
            self.indptr = indptr
...
def skeletons_to_tensorskeleton(skeletons_batch: List[Skeleton], device: str=None) -> TensorSkeleton:
    ...
    for batch_i, skeleton in enumerate(skeletons_batch):
        ...
        if first_processed:
            # Remove first item of indptr because it will be repeated by the last item of the previous indptr
            path_delim = skeleton.paths.indptr[1:]
        else:
            path_delim = skeleton.paths.indptr
        if skeleton.paths.indptr.shape[0] > 0:
            first_processed = True
        path_delim += path_delim_offset
        path_delim_list.append(path_delim)
        batch_list.append(batch_i * np.ones(n_points, dtype=int))
