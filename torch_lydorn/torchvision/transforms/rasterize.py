...
class Rasterize(object):
    ...
    def __call__(self, image, polygons):
        ...
        #alpha = 0.25
        #added_image = cv2.addWeighted(out[0], alpha, image, 1 - alpha, 0)
        #cv2.imwrite("mix.png", added_image[..., ::-1])
        #raise NotImplementedError()
        return out
...
def compute_raster_distances_sizes(polygons, shape, fill=True, edges=True, vertices=True, line_width=3, antialiasing=False):
    ...
    #skimage.io.imsave("distances.png", np.clip(distances * 255.0/distances.max(), 0, 255).astype(np.uint8))
...
def draw_polygons(polygons, shape, fill=True, edges=True, vertices=True, line_width=3, antialiasing=False):
    ...
    if fill:
        im_draw = rasterio.features.rasterize(polygons, draw_shape, default_value=255, all_touched=False)
        im_draw_list.append(im_draw)
    if edges:
        lines = [line.buffer(line_width / 2, cap_style='flat', join_style='bevel') for p in polygons for line in [p.exterior] + list(p.interiors)]
        im_draw = rasterio.features.rasterize(lines, draw_shape, default_value=255, all_touched=False)
        im_draw_list.append(im_draw)
    if vertices:
        points = [shapely.Point(vertex).buffer(line_width / 2) for polygon in polygons for vertex in list(polygon.exterior.coords) + \
                  [v for interior in list(polygon.interiors) for v in list(interior.coords)]]
        im_draw = rasterio.features.rasterize(points, draw_shape, default_value=255, all_touched=False)
        im_draw_list.append(im_draw)

    for i in range(len(im_draw_list), 3):
        im_draw_list.append(np.zeros(draw_shape, dtype=np.uint8))

    im_draw = np.stack(im_draw_list, axis=-1)

    if antialiasing:
        im_draw = Image.fromarray(im_draw)
        resize_shape = (shape[1], shape[0])
        im_draw = im_draw.resize(resize_shape, Image.BILINEAR)
        im_draw = np.array(im_draw)
